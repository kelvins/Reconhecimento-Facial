...
conteudo += "\nFalso Positivo (FP) : faces reconhecidas (abaixo do limiar) incorretamente. O algoritmo reconheceu porem nao eh o sujeito correto."
conteudo += "\nVerdadeiro Negativo (VN) : faces nao reconhecidas (acima do limiar) corretamente. Realmente nao eram para ser reconhecidas, ou porque o sujeito nao esta na base de treinamento ou porque nao corresponde a face de um sujeito."
...
for limiar in range(limiarMin, limiarMax+1, passo):
	...
	for index in xrange(0, len(listaDirTreinamento)):
		...
		#numSujeito = int(listaDirTreinamento[index].split("_")[0])
	...
	for index in xrange(0, len(listaDirTeste)):
		...
		if confianca <= limiar:
			...
			if sujeitoCLassificado == sujeitoEsperado:
				VP += 1
			# Caso contrario, se o algoritmo classificou um sujeito diferente esta incorreto (falso positivo)
			else:
				FP += 1
		# Se a confianca for maior que o limiar atual significa que o algoritmo nao reconheceu o sujeito
			...
			# ou realmente nao corresponde a face de um sujeito. Neste caso o algoritmo esta de certa forma correto ao nao identificar (verdadeiro negativo)
	...
	conteudo += str(limiar) + ";" + str(VP) + ";" + str(FP) + ";" + str(VN) + ";" + str(FN) + "\n";
